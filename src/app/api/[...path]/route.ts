  ...
  try {
    ...
    switch (resource) {
      ...
      case "courses": {
        ...
        if (id) {
          if (!courses[0]) throw new ApiError("Không tìm thấy khóa học.", 404);
          const [enrollment, progress] = user
            ? await Promise.all([
                ...
                db.lessonProgress.findMany({
                  ...
                  select: { lessonId: true, step: true, completed: true },
                }),
              ])
            : [null, []];
          data = {
            ...
            lessons: courses[0].lessons.map((lesson) => ({
              ...lesson,
              progress:
          ...
          const { page, pageSize, skip } = readPagination(
            url.searchParams,
            12,
            60,
          );
      ...
      case "vocabulary": {
        ...
        if (!id && url.searchParams.get("paginated") === "1") {
          ...
          const [words, total, categories, totalWords, learnedCount] =
            await Promise.all([
          ...
          const learned = await db.userVocabulary.findMany({
            where: {
              userId: user.id,
              vocabularyId: { in: words.map((word) => word.id) },
            },
          });
          data = {
            items: words.map((word) => ({
              ...word,
              review:
                learned.find((item) => item.vocabularyId === word.id) || null,
            })),
            pagination: paginationMeta(total, page, pageSize),
            facets: {
              categories: categories.map((item) => item.category),
        ...
        const learned = await db.userVocabulary.findMany({
          where: { userId: user.id, ...(id ? { vocabularyId: id } : {}) },
        });
        data = words.map((w) => ({
          ...w,
          review: learned.find((v) => v.vocabularyId === w.id) || null,
        }));
        break;
      }
      case "grammar":
        await requireUser();
        data = id
          ? await db.grammarLesson.findUnique({ where: { id } })
          : await db.grammarLesson.findMany({ orderBy: { id: "asc" } });
        break;
      case "listening":
        await requireUser();
        data = id
          ? await db.listeningLesson.findUnique({ where: { id } })
          : await db.listeningLesson.findMany();
        break;
      case "reading":
        await requireUser();
        data = id
          ? await db.readingArticle.findUnique({ where: { id } })
          : await db.readingArticle.findMany();
        break;
      case "writing": {
        const user = await requireUser();
        data =
          id === "submissions"
            ? await db.writingSubmission.findMany({
                ...
                include: {
                  exercise: { select: { id: true, title: true, level: true } },
                },
      ...
      case "favorites":
        data = await db.favorite.findMany({
          where: { userId: (await requireUser()).id },
          orderBy: { createdAt: "desc" },
        });
        break;
      case "notes":
        data = await db.note.findMany({
          where: { userId: (await requireUser()).id },
      ...
      case "ai": {
        const user = await requireUser();
        if (id === "status") data = await getAIStatus(user.id);
        else
          data = id
      ...
      case "leaderboard": {
        ...
        const days =
          { daily: 1, weekly: 7, monthly: 30, all: 36500 }[period as "daily"] ||
          7;
        const users = await db.user.findMany({
          where: { banned: false, profile: { publicLeaderboard: true } },
          select: {
            id: true,
            name: true,
            image: true,
            profile: { select: { level: true, country: true } },
            progress: { select: { xp: true } },
            dailyGoals: {
              where: {
                date: {
                  gte: studyDate(new Date(Date.now() - (days - 1) * 86400000)),
                },
              },
            },
          },
        ...
        data = users
          .map((u) => ({
            ...
            xp:
              period === "all"
                ? u.progress?.xp || 0
                : u.dailyGoals.reduce((s, g) => s + g.xp, 0),
          }))
          .sort((a, b) => b.xp - a.xp)
          .slice(0, 50);
        break;
      ...
      case "search": {
        ...
        const [words, courses, lessons, grammar, reading] = await Promise.all([
          db.vocabulary.findMany({
            where: {
              OR: [
        ...
        data = [
          ...words.map((w) => ({
            ...
            subtitle: w.meaning,
            type: "Từ vựng",
            href: `/dictionary?q=${encodeURIComponent(w.word)}`,
          })),
          ...courses.map((c) => ({
            id: c.id,
            title: c.title,
    ...
    if (data === null) throw new ApiError("Không tìm thấy nội dung.", 404);
...
async function mutate(request: Request, { params }: Context) {
  try {
    ...
    switch (resource) {
      case "onboarding": {
        ...
        data = await db.$transaction(async (tx) => {
          await tx.profile.upsert({
            where: { userId: user.id },
            create: { userId: user.id, ...values, onboardingComplete: true },
            update: { ...values, onboardingComplete: true },
          });
          return tx.learningPlan.upsert({
            where: { userId: user.id },
            create: {
              userId: user.id,
              level: values.level,
            ...
            update: { level: values.level, goal: values.goal },
          });
        });
        break;
      }
      case "courses": {
        if (action !== "enroll") throw new ApiError("Hành động không hợp lệ.");
        const course = await db.course.findFirst({
          where: { id, published: true },
        });
        if (!course) throw new ApiError("Không tìm thấy khóa học.", 404);
        if (
          course.premium &&
          user.role !== "ADMIN" &&
          !(
        ...
        data = await db.courseEnrollment.upsert({
          where: { userId_courseId: { userId: user.id, courseId: id } },
          create: { userId: user.id, courseId: id },
          update: {},
        });
        break;
      }
      case "lessons": {
        ...
        data = await db.$transaction(async (tx) => {
          ...
          return tx.lessonProgress.upsert({
            where: { userId_lessonId: { userId: user.id, lessonId: id } },
            create: { userId: user.id, lessonId: id, step },
            update: { step: Math.max(step, current?.step || 0) },
          });
        });
        break;
      }
      ...
      case "quiz": {
        if (id !== "submit") throw new ApiError("Hành động không hợp lệ.");
        const values = z
          .object({
            quizId: z.string(),
            answers: z
      ...
      case "study": {
        if (id === "start") {
          const v = z
            .object({
              kind: z.enum(["lesson", "speaking", "reading", "listening"]),
              resourceId: z.string().max(100),
            })
            .parse(raw);
          if (v.kind === "lesson") await lessonData(v.resourceId, user.id);
          data = await db.studySession.create({
            data: { userId: user.id, ...v },
          });
        } else if (id === "finish") {
      ...
      case "favorites": {
        const v = z
          .object({
            type: z.enum(["word", "grammar", "lesson", "article"]),
            resourceId: z.string().max(100),
            title: z.string().max(150),
            href: z
              .string()
              .regex(
                /^\/(vocabulary|dictionary|grammar|lessons|reading|courses)(\/[^\s?#]*)?(\?[^\s]*)?$/,
              ),
          })
          .parse(raw);
        const where = {
      ...
      case "notes": {
        if (id) {
          ...
          if (!note) throw new ApiError("Ghi chú không tồn tại.", 404);
          data =
            request.method === "DELETE"
              ? await db.note.delete({ where: { id } })
              : await db.note.update({
                  where: { id },
                  data: noteSchema.parse(raw),
                });
        } else
          data = await db.note.create({
            data: { userId: user.id, ...noteSchema.parse(raw) },
          });
        break;
      }
      case "profile":
        ...
        break;
      case "subscription": {
        if (id !== "interest") throw new ApiError("Hành động không hợp lệ.");
        const v = z
          .object({ period: z.enum(["monthly", "yearly"]) })
          .parse(raw);
        data = await db.note.create({
          data: {
            userId: user.id,
            title: "Quan tâm EnglishMaster Premium",
            content: `Tôi quan tâm gói ${v.period === "yearly" ? "hằng năm" : "hằng tháng"}. Chưa có giao dịch hay thanh toán.`,
      ...
      case "notifications":
        data = await db.notification.updateMany({
          where: {
            userId: user.id,
            ...(id && id !== "read-all" ? { id } : {}),
          },
      ...
      case "ai":
        if (request.method === "DELETE" && id && !action) {
          const removed = await db.aIConversation.deleteMany({
            where: { id, userId: user.id },
          });
          if (!removed.count)
            throw new ApiError("Không tìm thấy hội thoại.", 404);
          data = { deleted: true };
        } else if (id === "chat") data = await chat(user.id, raw);
        else if (id === "recommendation")
          data = await recommendLearning(user.id);
        else throw new ApiError("Hành động không hợp lệ.");
        break;
      case "writing":
