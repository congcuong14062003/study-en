...
import {
  ...
  Favorites,
  Notifications,
} from "@/components/account/collections";
import {
  ProgressPages,
  Leaderboard,
} from "@/components/account/progress-pages";
import { ProfileSettings } from "@/components/account/profile-settings";
import { LoadingSkeleton } from "@/components/ui/states";
export const dynamic = "force-dynamic";
const LessonPlayer = nextDynamic(
  () =>
    import("@/components/learning/lesson-player").then((m) => m.LessonPlayer),
  { loading: () => <LoadingSkeleton /> },
);
const Speaking = nextDynamic(
  () => import("@/components/learning/speaking").then((m) => m.Speaking),
  { loading: () => <LoadingSkeleton /> },
);
const Writing = nextDynamic(
  () => import("@/components/learning/writing").then((m) => m.Writing),
  { loading: () => <LoadingSkeleton /> },
);
...
export default async function Page({ params, searchParams }: Props) {
  ...
  const id = slug?.[0];
  if (!(section in routeTitles) || (slug && slug.length > 1)) notFound();
  if (
    id &&
    ![
      "courses",
  ...
  if (!user && section !== "courses")
    redirect(
      `/login?callbackUrl=${encodeURIComponent(`/${section}${id ? `/${id}` : ""}`)}`,
    );
  ...
  if (section === "admin" && user?.role !== "ADMIN") redirect("/forbidden");
  ...
  switch (section) {
    ...
    case "quiz":
      content = (
        <QuizPage
          quizId={`practice-${["vocabulary", "grammar", "reading", "listening"].includes(String(query.type)) ? query.type : "vocabulary"}`}
        />
      );
      break;
    case "placement-test":
      ...
      user={
        user
          ? {
              ...
              email: user.email,
              role: user.role,
              profile: user.profile ? { level: user.profile.level } : null,
            }
          : null
      }
    >
