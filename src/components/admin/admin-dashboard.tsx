...
export function AdminDashboard({ section = "dashboard" }: {
    ...
    }, [debouncedQuery, filter, pageSize, paginatedSection, requestPage, section]);
    ...
    async function save(e: React.FormEvent) {
        e.preventDefault();
        setFormError("");
        const missingField = requiredFields[section]?.find(key => typeof form[key] === "string" && !String(form[key]).trim());
        if (missingField) {
            const fieldLabel = String(labels[missingField] || missingField).replace(" (tùy chọn)", "");
    ...
    async function remove() {
        ...
        try {
            ...
            setDeleting(null);
            if (paginatedSection && data && isPaginatedRows(data) && data.items.length === 1 && page > 1)
    ...
    const rows = pageData ? collection : collection.filter(r => JSON.stringify(r).toLowerCase().includes(q.toLowerCase()) && (filter === "all" || r.level === filter));
