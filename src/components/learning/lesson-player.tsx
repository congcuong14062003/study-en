...
export function LessonPlayer({ id }: { id: string }) {
  const { data, loading, error, refresh } = useData<LessonData>(
    `/lessons/${id}`,
  );
  ...
  useEffect(() => {
    ...
    api<{
      ...
      method: "POST",
      body: JSON.stringify({ kind: "lesson", resourceId: id }),
    })
      .then((s) => {
        if (active) session.current = s.id;
        else
  ...
  const steps = [
    { label: "Ghép từ", icon: Sparkles },
    { label: "Xếp câu", icon: GripVertical },
    { label: "Nghe", icon: Headphones },
    { label: "Nói", icon: Mic },
    { label: "Đọc & viết", icon: BookOpen },
    { label: "Thử thách", icon: PenLine },
  ];
  const nextLesson = data.lesson.course.lessons.find(
    (l) => l.order > data.lesson.order,
  );
  const sentence =
    ...
    <>
      <div className="lesson-heading">
        <div>
          ...
          <Badge className="green">BÀI {data.lesson.order} · 6 HOẠT ĐỘNG</Badge>
        ...
        <div className="flex-row">
          <FavoriteButton
            ...
            href={`/lessons/${id}`}
          />
          <Button variant="outline" size="sm" onClick={() => setNote(true)}>
            <NotebookPen size={16} />
            Ghi chú
          </Button>
        </div>
      </div>
      <nav className="lesson-stepper" aria-label="Các phần bài học">
        {steps.map(({ label, icon: Icon }, i) => (
          <button
            className={`${i === step ? "active" : ""} ${i < maxStep ? "done" : ""}`}
            disabled={i > maxStep}
            onClick={() => setStep(i)}
            key={label}
          >
            <span>{i < maxStep ? <Check size={14} /> : i + 1}</span>
            <Icon size={16} />
            {label}
          </button>
        ))}
      </nav>
      <Progress value={((finished ? 6 : step) / 6) * 100} />
      <div className="lesson-content interactive-lesson">
          ...
          <div className="content-narrow">
            ...
            {data.grammar && (
              <Card className="grammar-bridge">
                <div>
                  <Badge>
                    <FileText size={12} /> CẤU TRÚC TRONG BÀI
                  </Badge>
                  <h2>{data.grammar.title}</h2>
                  <p>{data.grammar.description}</p>
                </div>
                <div className="grammar-bridge-formula">
                  {data.grammar.structure.slice(0, 2).map((item) => (
            ...
            {data.listening ? (
              <Card className="section-card lesson-skill-card">
                <div className="skill-tip">
                  <Headphones size={18} />
                  <span>
                    Nghe lần đầu không xem lời thoại. Lần hai mở transcript và
                    đọc theo.
                  </span>
                </div>
                <MediaAudioPlayer
                  audioUrl={data.listening.audioUrl}
                  text={data.listening.transcript}
                  translation={data.listening.translation}
                  title={data.listening.title}
                />
              </Card>
            ) : (
              <Card className="activity-card activity-empty">
                <Headphones size={35} />
                <h2>Bài này chưa có đoạn nghe riêng</h2>
                <p>Bạn có thể để phần này lại và chuyển sang luyện nói.</p>
              </Card>
            )}
          </div>
        )}
        {step === 3 && (
          <div className="content-narrow">
            <SpeakingRecorder text={sentence} />
          </div>
        )}
        {step === 4 && (
          <div className="content-narrow">
            {data.reading && (
              <>
                <h2 className="mb-4">{data.reading.title}</h2>
            ...
            <MiniWriting
              ...
              suggestedWords={data.words.slice(0, 4).map((word) => word.word)}
            />
          </div>
        )}
        {step === 5 && (
      ...
      <div className="lesson-bottom">
        ...
        >
          <ArrowLeft size={15} /> Phần trước
        </Button>
        <span>
          {finished ? "Đã hoàn thành bài học" : `Hoạt động ${step + 1} trên 6`}
        </span>
        {step < 5 ? (
      ...
      <Dialog open={note} onOpenChange={setNote}>
        <DialogContent>
          <DialogTitle>Lưu một điều đáng nhớ</DialogTitle>
          <DialogDescription>
            Ghi chú này sẽ được lưu trong Góc của bạn.
          </DialogDescription>
          <form onSubmit={saveNote}>
            ...
            <div className="field">
              ...
              <textarea
                ...
                required
                maxLength={12000}
                placeholder="Một cấu trúc mới, một ví dụ hay…"
              />
            </div>
