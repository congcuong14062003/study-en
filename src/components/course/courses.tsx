...
type CoursePage = PaginatedResponse<
  CourseRecord,
  {
    categories: string[];
    lessonCount: number;
  ...
  return (
    <div className="course-path">
      {units.map((lessons, unitIndex) => (
        <section className="path-unit" key={unitIndex}>
          <div className="path-unit-heading">
            ...
            <Badge className="green">
              <Trophy size={12} />{" "}
              {lessons.filter((lesson) => lesson.progress?.completed).length}/
              {lessons.length}
            </Badge>
          </div>
          <div className="path-track">
            {lessons.map((lesson, localIndex) => {
              ...
              return course.enrolled ? (
                ...
                <div
                  className={`path-node bend-${localIndex % 3} locked`}
                  key={lesson.id}
                >
                  {node}
                </div>
...
export function CourseCard({ course }: { course: CourseRecord }) {
  ...
  return (
    <Link href={`/courses/${course.id}`} className="course-card card">
      ...
      <div className="course-card-body">
        ...
        <h3>{course.title}</h3>
        <p>{course.description.slice(0, 112)}…</p>
        <span className="text-link">
          Khám phá khóa học <ArrowRight size={15} />
        </span>
      </div>
    </Link>
  );
}
export function Courses({ id, userId }: { id?: string; userId?: string }) {
  ...
  const { data, loading, error, refresh } = useData<CoursePage | CourseRecord>(
    id ? `/courses/${id}` : listPath,
  );
  ...
  if (id && !("items" in data)) {
    ...
    async function enroll() {
      if (!userId) {
        router.push(
          `/login?callbackUrl=${encodeURIComponent(`/courses/${id}`)}`,
        );
        return;
      }
      setBusy(true);
      try {
        ...
        router.push(
          `/lessons/${c.lessons.find((lesson) => !lesson.progress?.completed)?.id || c.lessons[0]?.id}`,
        );
      } catch (e) {
    ...
    return (
      <>
        ...
        <div className="course-detail-layout">
          <div>
            <Card className={`content-header course-detail-hero ${c.color}`}>
              <Badge>
                {c.level} · {c.category}
              </Badge>
              <h1>{c.title}</h1>
              ...
              <div className="course-detail-meta">
                ...
                <span>
                  <Star size={15} />
                  {c.rating} · Nội dung mẫu
                </span>
              </div>
            </Card>
            <Card className="section-card">
              <h2>Bạn sẽ làm được gì?</h2>
          ...
          <aside>
            <Card className="section-card course-enroll-card">
              <span className="icon-box purple">
                <GraduationCap size={30} />
              </span>
              <h2>{c.premium ? "Khóa học Premium" : "Miễn phí để bắt đầu"}</h2>
              <p>Học theo nhịp của bạn, quay lại bất cứ lúc nào.</p>
              <Button
                className="w-full mt-4"
                disabled={busy || !c.lessons.length}
                onClick={enroll}
              >
                {busy ? (
              ...
              <ul className="list-clean">
                <li>
                  ...
                  Mỗi chặng gồm 6 bài tương tác
                </li>
                <li>
                  <CheckCircle2 size={16} />
                  Kéo thả từ và sắp xếp câu
                </li>
                <li>
                  <CheckCircle2 size={16} />
                  Lưu tiến độ tự động
  ...
  if (!("items" in data))
    return (
      <ErrorState
        message="Dữ liệu khóa học không đúng định dạng."
        retry={refresh}
      />
    );
  ...
  return (
    <>
      <PageHeading
        ...
        description="Khám phá những khóa học được xây dựng cho hành trình của riêng bạn."
      >
        <Badge>
          <BookOpen size={12} /> {data.pagination.total} khóa học ·{" "}
          {data.facets.lessonCount} bài học
        </Badge>
      </PageHeading>
      <div className="course-banner">
        <div>
          ...
          <p>Từ lời chào đầu tiên đến một cuộc họp tự tin.</p>
        </div>
      ...
      <div className="toolbar">
        ...
        <select
          aria-label="Lọc danh mục khóa học"
          value={category}
          onChange={(e) => {
      ...
      {courses.length ? (
        <>
          <MotionGrid className="course-grid">
            {courses.map((course) => (
              <MotionItem className="motion-card-shell" key={course.id}>
        ...
        <EmptyState
          ...
          description="Thử một từ khóa hoặc trình độ khác nhé."
        />
      )}
    </>
  );
