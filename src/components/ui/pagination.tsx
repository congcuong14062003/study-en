...
function pageTokens(page: number, totalPages: number): PageToken[] {
  ...
  const pages = [
    ...new Set([1, 2, page - 1, page, page + 1, totalPages - 1, totalPages]),
  ]
  ...
  return (
    <nav className="pagination" aria-label="Phân trang">
      <p className="pagination-summary">
        ...
        trong tổng số <strong>{meta.total}</strong> kết quả
      </p>
        ...
        <div className="pagination-size-picker" ref={sizePickerRef}>
          ...
          {sizeMenuOpen && (
            <div
              ...
              role="listbox"
              aria-label="Chọn số kết quả mỗi trang"
            >
              {pageSizes.map((size) => (
                <button
                  type="button"
                  role="option"
                  aria-selected={size === meta.pageSize}
                  className={size === meta.pageSize ? "active" : ""}
                  key={size}
                  onClick={() => {
                ...
                >
                  <span>{size}</span>
                  {size === meta.pageSize && <Check size={14} />}
                </button>
              ))}
            </div>
          )}
      ...
      <div className="pagination-pages">
        ...
        {pageTokens(meta.page, meta.totalPages).map((token) =>
          typeof token === "number" ? (
            <button
              type="button"
              key={token}
              className={token === meta.page ? "active" : ""}
            ...
            <span className="pagination-gap" key={token}>
              …
            </span>
          ),
        )}
