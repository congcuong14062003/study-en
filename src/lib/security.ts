...
export function checkOrigin(request: Request) {
  ...
  if (
    receivedUrl.origin === expectedUrl.origin ||
    receivedUrl.origin === receivedRequestOrigin
  )
    return;
  const localHosts = new Set(["localhost", "localhost", "[::1]"]);
...
export async function readJson(request: Request) {
  ...
  if (!reader) throw new ApiError("Thiếu nội dung JSON.");
  let size = 0;
