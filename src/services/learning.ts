...
export async function reward(
  ...
  await tx.dailyGoal.upsert({
    ...
    create: {
      userId,
      date: today,
      targetMinutes: profile?.dailyMinutes || 20,
      seconds,
      xp,
    },
  ...
  if (kind !== "time")
    await tx.studySession.create({
      data: { userId, kind, resourceId, xp, seconds, endedAt: new Date() },
    });
  const badges = [
    ...(updated.lessonsCompleted >= 1 ? ["first-lesson"] : []),
    ...(streak >= 7 ? ["streak-7"] : []),
    ...(streak >= 30 ? ["streak-30"] : []),
    ...(updated.wordsLearned >= 30 ? ["vocabulary-30"] : []),
    ...(updated.wordsLearned >= 1000 ? ["words-1000"] : []),
    ...(updated.lessonsCompleted >= 100 ? ["lessons-100"] : []),
  ];
  for (const achievementId of badges) {
    const owned = await tx.userAchievement.findUnique({
      where: { userId_achievementId: { userId, achievementId } },
    });
    if (!owned) {
      await tx.userAchievement.create({ data: { userId, achievementId } });
      const badge = await tx.achievement.findUnique({
      ...
      await tx.notification.create({
        data: {
          userId,
          type: "achievement",
          title: `Thành tích mới: ${badge?.title}`,
          body: "Bạn vừa mở khóa một cột mốc mới. Tiếp tục phát huy nhé!",
          href: "/achievements",
        },
      });
    }
...
export async function reviewVocabulary(
  ...
  return db.$transaction(async (tx) => {
    ...
    if (!word) throw new ApiError("Không tìm thấy từ vựng.", 404);
    const current = await tx.userVocabulary.findUnique({
      where: { userId_vocabularyId: { userId, vocabularyId: id } },
    });
    if (current && current.dueAt > new Date())
      throw new ApiError(
        "Bạn đã ôn từ này. Hãy quay lại khi đến lịch ôn.",
        409,
      );
    const state = scheduleReview(
      current || { repetitions: 0, interval: 0, ease: 2.5 },
      rating,
    );
    await tx.userVocabulary.upsert({
      where: { userId_vocabularyId: { userId, vocabularyId: id } },
      create: { userId, vocabularyId: id, ...state },
      update: state,
    });
    await tx.vocabularyReview.create({
      data: { userId, vocabularyId: id, rating },
    });
    if (!current)
...
/** Makes a vocabulary item available in the learner's current flashcard session. */
export async function startVocabulary(userId: string, id: string) {
  return db.$transaction(async (tx) => {
    await lockUser(tx, userId);
    const word = await tx.vocabulary.findUnique({ where: { id } });
    if (!word) throw new ApiError("Không tìm thấy từ vựng.", 404);
    ...
    await tx.userVocabulary.upsert({
      where: { userId_vocabularyId: { userId, vocabularyId: id } },
      create: { userId, vocabularyId: id, dueAt: new Date() },
      update: { dueAt: new Date() },
    });
    if (!current)
      await tx.userProgress.upsert({
        where: { userId },
        create: { userId, wordsLearned: 1 },
...
/** Removes a word from a learner's active vocabulary and review queue. */
export async function removeVocabulary(userId: string, id: string) {
  return db.$transaction(async (tx) => {
    await lockUser(tx, userId);
    const removed = await tx.userVocabulary.deleteMany({
      where: { userId, vocabularyId: id },
    });
    if (!removed.count) throw new ApiError("Từ này chưa có trong danh sách đang học.", 404);
    ...
    await tx.userProgress.update({
      where: { userId },
      data: { wordsLearned: Math.max(0, progress.wordsLearned - 1) },
    });
    return { removed: true };
  });
}

const STARTER_FLASHCARD_COUNT = 30;

/** Adds an immediately-due, beginner-friendly set of cards for a new learner. */
export async function addStarterFlashcards(userId: string) {
  return db.$transaction(async (tx) => {
    await lockUser(tx, userId);
    const vocabulary = await tx.vocabulary.findMany({
      where: {
        level: { in: ["A1", "A2"] },
        users: { none: { userId } },
      },
      orderBy: [{ level: "asc" }, { word: "asc" }],
      take: STARTER_FLASHCARD_COUNT,
      select: { id: true },
    });
    if (!vocabulary.length)
      throw new ApiError("Bạn đã thêm toàn bộ từ trong bộ khởi động.", 409);

    await tx.userVocabulary.createMany({
      data: vocabulary.map(({ id: vocabularyId }) => ({
        userId,
        vocabularyId,
        dueAt: new Date(),
      })),
    ...
    await tx.userProgress.upsert({
      where: { userId },
      create: { userId, wordsLearned: vocabulary.length },
      update: { wordsLearned: { increment: vocabulary.length } },
    });
    return { added: vocabulary.length };
  });
}
export async function submitQuiz(
  ...
  return db.$transaction(
    async (tx) => {
      ...
      if (!quiz) throw new ApiError("Bài kiểm tra không tồn tại.", 404);
      if (
        quiz.lesson &&
        (!quiz.lesson.published || !quiz.lesson.course.published)
      )
      ...
      if (quiz.lesson) {
        ...
        if (!lp || lp.step < 5)
          throw new ApiError(
            "Hãy hoàn thành các phần trước khi làm bài kiểm tra.",
            409,
          );
      }
      if (
        ...
        answers.length !== quiz.questionIds.length ||
        answers.some((a) => !quiz.questionIds.includes(a.questionId))
      )
      ...
      if (!questions.length || questions.length !== quiz.questionIds.length)
        throw new ApiError(
          "Bài kiểm tra thiếu câu hỏi. Vui lòng liên hệ quản trị viên.",
          409,
        );
      ...
      const previous = await tx.quizAttempt.findFirst({
        where: { userId, quizId },
      });
      let xp = previous ? 0 : score === total ? 30 : 20;
      if (quiz.lesson && percent < 60) xp = 0;
      const attempt = await tx.quizAttempt.create({
        data: {
          ...
          xp,
          answers: {
            create: results.map(({ questionId, selected, correct }) => ({
              questionId,
              selected,
              correct,
            })),
      ...
      if (quiz.lesson && percent >= 60) {
        ...
        if (!existing?.completed) {
          await tx.lessonProgress.upsert({
            ...
            update: { step: 6, completed: true, completedAt: new Date() },
          });
          await tx.userProgress.update({
            where: { userId },
      ...
      for (const skill of ["vocabulary", "grammar", "reading", "listening"]) {
        ...
        if (r.length)
          skillScores[skill] = Math.round(
            (r.filter((x) => x.correct).length / r.length) * 100,
          );
      }
      ...
      await tx.userProgress.update({
        ...
        data: {
          skillScores: {
            ...((progress?.skillScores as Record<string, number>) || {}),
            ...skillScores,
          },
        },
      });
      if (quiz.kind === "placement") {
        ...
        await tx.profile.update({
          where: { userId },
          data: { level, placementComplete: true },
        });
