...
export const cmsSchemas = {
  ...
  vocabulary: z.object({
    word: str,
    ipa: z.string().max(200).optional().default(""),
    meaning: str,
    definition: z
      .string()
      .trim()
      .min(1, "Vui lòng nhập định nghĩa tiếng Anh.")
      .max(12000),
    partOfSpeech: str,
    example: str,
    translation: str,
  ...
  questions: z
    ...
    .refine(
      (v) => v.correctAnswer < v.options.length,
      "Chỉ số đáp án phải nằm trong danh sách lựa chọn.",
    ),
  lessons: z.object({
    courseId: str,
    title: str,
    ...
    vocabularyIds: strings,
    grammarId: z.string().nullable().optional(),
    listeningId: z.string().nullable().optional(),
...
export async function adminRead(
  ...
  switch (section) {
    ...
    case "grammar":
      return db.grammarLesson.findMany();
    case "listening":
      return db.listeningLesson.findMany();
    case "reading":
      return db.readingArticle.findMany();
    case "questions":
...
export async function adminWrite(
  ...
  if (section === "users") {
    if (!id) throw new ApiError("Thiếu người dùng.");
    if (id === adminId)
      throw new ApiError(
        "Không thể thay đổi quyền hoặc xóa tài khoản quản trị đang sử dụng.",
      );
    const target = await db.user.findUnique({ where: { id } });
    if (!target) throw new ApiError("Không tìm thấy người dùng.", 404);
    if (target.role === "ADMIN")
      throw new ApiError(
        "Hãy quản lý tài khoản quản trị khác qua quy trình quản trị máy chủ.",
        403,
      );
    if (method === "DELETE")
    ...
    return db.$transaction(async (tx) => {
      await tx.subscription.upsert({
        ...
        create: {
          userId: id,
          plan: parsed.role === "PREMIUM" ? "PREMIUM" : "FREE",
          status: "active",
        },
        update: {
          plan: parsed.role === "PREMIUM" ? "PREMIUM" : "FREE",
      ...
      return tx.user.update({
        where: { id },
        data: { ...parsed, sessionVersion: { increment: 1 } },
        select: { id: true },
      });
    });
  }
  ...
  if (method === "DELETE") {
    if (!id) throw new ApiError("Thiếu nội dung cần xóa.");
    switch (section) {
      case "courses":
        if (await db.courseEnrollment.count({ where: { courseId: id } }))
          throw new ApiError(
            "Khóa học đã có học viên. Hãy chuyển về bản nháp.",
            409,
          );
  ...
  switch (section) {
    case "courses": {
      ...
      return id
        ? db.course.update({ where: { id }, data })
    ...
    case "vocabulary": {
      ...
      return id
        ? db.vocabulary.update({ where: { id }, data: vocabulary })
        : db.vocabulary.create({ data: vocabulary });
    }
    case "grammar": {
      const data = cmsSchemas.grammar.parse(raw);
      return id
        ? db.grammarLesson.update({ where: { id }, data })
        : db.grammarLesson.create({ data });
    }
    case "listening": {
      const data = cmsSchemas.listening.parse(raw);
      return id
        ? db.listeningLesson.update({ where: { id }, data })
        : db.listeningLesson.create({ data });
    }
    case "reading": {
      const data = cmsSchemas.reading.parse(raw);
      return id
        ? db.readingArticle.update({ where: { id }, data })
        : db.readingArticle.create({ data });
    }
    case "questions": {
      const data = cmsSchemas.questions.parse(raw);
      return id
        ? db.question.update({ where: { id }, data })
        : db.question.create({ data });
    }
    case "lessons": {
      const data = cmsSchemas.lessons.parse(raw);
      const [words, questions, grammar, listening, reading] = await Promise.all(
        [
          db.vocabulary.count({ where: { id: { in: data.vocabularyIds } } }),
          db.question.count({ where: { id: { in: data.questionIds } } }),
          data.grammarId
            ? db.grammarLesson.findUnique({ where: { id: data.grammarId } })
            : true,
          data.listeningId
            ? db.listeningLesson.findUnique({ where: { id: data.listeningId } })
            : true,
          data.readingId
            ? db.readingArticle.findUnique({ where: { id: data.readingId } })
            : true,
        ],
      );
      if (
        words !== data.vocabularyIds.length ||
        ...
        throw new ApiError(
          "Tham chiếu nội dung không tồn tại, trùng lặp hoặc thiếu câu hỏi.",
        );
      return db.$transaction(async (tx) => {
        const lesson = id
        ...
        await tx.quiz.upsert({
          ...
          update: { title: lesson.title, questionIds: lesson.questionIds },
        });
        return lesson;
      });
    }
