...
export async function lessonData(id: string, userId: string) {
  ...
  if (!lesson) throw new ApiError("Không tìm thấy bài học.", 404);
  await requireCourseAccess(userId, lesson.courseId);
  const [words, grammar, listening, reading, progress] = await Promise.all([
    db.vocabulary.findMany({ where: { id: { in: lesson.vocabularyIds } } }),
    lesson.grammarId
      ? db.grammarLesson.findUnique({ where: { id: lesson.grammarId } })
      : null,
    lesson.listeningId
      ? db.listeningLesson.findUnique({ where: { id: lesson.listeningId } })
      : null,
    lesson.readingId
      ? db.readingArticle.findUnique({ where: { id: lesson.readingId } })
      : null,
  ...
  return {
    ...
    reading,
    progress,
    quiz: await publicQuiz(`quiz-${id}`, userId),
  };
}
