...
export async function forgotPassword(request: Request) {
  try {
    checkOrigin(request);
    const { email } = z
      .object({ email: z.email().toLowerCase().trim() })
    ...
    if (user && !user.banned) {
      ...
      await transport.sendMail({
        ...
        to: email,
        subject: "Đặt lại mật khẩu EnglishMaster",
        text: `Mở liên kết sau để đặt lại mật khẩu trong 30 phút: ${origin}/reset-password?token=${token}\nNếu bạn không yêu cầu, hãy bỏ qua email này.`,
      });
    }
    return Response.json({
      message:
        "Nếu email này đã được đăng ký, bạn sẽ nhận được hướng dẫn khôi phục.",
    });
  } catch (e) {
    return errorResponse(e);
  }
  ...
  try {
    ...
    await db.$transaction(async (tx) => {
      ...
      const deleted = await tx.passwordReset.deleteMany({
        where: { id: saved.id, expiresAt: { gt: new Date() } },
      });
      if (deleted.count !== 1) throw new ApiError("Liên kết đã được sử dụng.");
