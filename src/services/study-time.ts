...
export function uncoveredSeconds(
  ...
  for (const interval of intervals) {
    if (interval.start > cursor) uncovered += interval.start - cursor;
  ...
  return db.$transaction(async (tx) => {
    ...
    const session = await tx.studySession.findFirst({
      where: {
        id: sessionId,
        userId,
        kind: { in: ["lesson", "speaking", "reading", "listening"] },
      },
    });
    if (!session) throw new ApiError("Phiên học không tồn tại.", 404);
    if (session.endedAt) return { seconds: session.seconds };
    const now = new Date();
    const start = Math.max(
      session.startedAt.getTime(),
      now.getTime() - 3600000,
    );
    const overlapping = await tx.studySession.findMany({
      where: {
        userId,
        id: { not: sessionId },
        kind: { in: ["lesson", "speaking", "reading", "listening"] },
        seconds: { gt: 0 },
        endedAt: { gt: new Date(start) },
        startedAt: { lt: now },
      },
      select: { startedAt: true, endedAt: true },
    });
    const seconds = uncoveredSeconds(
      start,
      now.getTime(),
      overlapping.map((s) => ({
        start: Math.max(s.startedAt.getTime(), s.endedAt!.getTime() - 3600000),
