...
async function main() {
  if (!process.argv.includes("--confirm-cloud"))
    throw new Error(
      "Pass --confirm-cloud to seed the configured remote database.",
    );
  const databaseUrl = process.env.DATABASE_URL;
  if (!databaseUrl) throw new Error("DATABASE_URL is missing.");
  const databaseHost = new URL(databaseUrl).hostname;
  if (["localhost", "localhost", "::1"].includes(databaseHost))
    throw new Error(
      "Refusing to run the cloud test seed against a local database.",
    );
  ...
  if (
    ...
    throw new Error(
      "Curriculum is empty. Run the content seed before creating test activity.",
    );
  ...
  for (let index = 1; index <= USER_COUNT; index++) {
    ...
    await db.userProgress.upsert({
      ...
      create: {
        ...
        skillScores: {
          vocabulary: 45 + (index % 50),
          grammar: 40 + (index % 55),
          listening: 35 + (index % 60),
          speaking: 30 + (index % 65),
          reading: 42 + (index % 50),
          writing: 38 + (index % 55),
        },
      },
    ...
    await db.learningPlan.upsert({
      ...
      create: {
        ...
        goal,
        weeks: [
          { week: 1, topic: "Daily routines" },
          { week: 2, topic: "Work and study" },
          { week: 3, topic: "Travel" },
  ...
  for (let index = 1; index <= USER_COUNT; index++) {
    ...
    for (
      let courseIndex = 0;
      courseIndex < Math.min(courses.length, 1 + (index % 3));
      courseIndex++
    ) {
      const course = courses[(courseIndex + index) % courses.length];
      enrollments.push({
        id: `seed-enrollment-${suffix}-${course.id}`,
        userId,
        courseId: course.id,
        createdAt: dateAgo(35 - courseIndex * 5),
      });
    }
    for (
      let lessonIndex = 0;
      lessonIndex < Math.min(lessons.length, 2 + (index % 7));
      lessonIndex++
    ) {
      const lesson = lessons[lessonIndex];
      const completed = lessonIndex < 1 + (index % 5);
      lessonProgress.push({
        id: `seed-lesson-progress-${suffix}-${lesson.id}`,
        userId,
        lessonId: lesson.id,
        step: completed ? 5 : 1 + (index % 4),
        completed,
        completedAt: completed ? dateAgo(lessonIndex + (index % 8)) : null,
        updatedAt: dateAgo(lessonIndex),
      });
    }
    for (
      let wordIndex = 0;
      wordIndex < Math.min(vocabulary.length, 8 + (index % 16));
      wordIndex++
    ) {
      const word = vocabulary[(wordIndex + index) % vocabulary.length];
      userVocabulary.push({
        id: `seed-user-word-${suffix}-${word.id}`,
        userId,
        vocabularyId: word.id,
        repetitions: wordIndex % 6,
        interval: wordIndex % 12,
        ease: 2.1 + (wordIndex % 5) / 10,
        dueAt: dateAgo((wordIndex % 7) - 3),
        lastReviewedAt: dateAgo(wordIndex % 10),
      });
      if (wordIndex < 5)
        vocabularyReviews.push({
          id: `seed-review-${suffix}-${word.id}`,
          userId,
          vocabularyId: word.id,
          rating: ["again", "hard", "good", "easy"][(wordIndex + index) % 4],
          reviewedAt: dateAgo(wordIndex),
        });
    }
    for (
      let attemptIndex = 0;
      attemptIndex < Math.min(3, quizzes.length);
      attemptIndex++
    ) {
      ...
      for (const [answerIndex, questionId] of selectedQuestionIds.entries()) {
        ...
        if (correct) score++;
        answers.push({
          id: `seed-answer-${suffix}-${attemptIndex}-${questionId}`,
          attemptId,
          questionId,
          selected,
          correct,
      ...
      attempts.push({
        ...
        xp: score * 10,
        createdAt: dateAgo(attemptIndex * 3 + (index % 12)),
      });
    }
    for (let day = 0; day < DAYS; day++) {
      ...
      dailyGoals.push({
        id: `seed-goal-${suffix}-${day}`,
        userId,
        date: dateKey(date),
        targetMinutes: 20 + (index % 3) * 10,
        seconds,
        xp: Math.floor(seconds / 30),
      });
      if (active && day < 15)
        sessions.push({
          id: `seed-session-${suffix}-${day}`,
          userId,
          kind: ["lesson", "speaking", "reading", "listening"][day % 4],
          resourceId: lessons[day % lessons.length].id,
          startedAt: date,
          endedAt: new Date(date.getTime() + seconds * 1000),
          seconds,
          xp: Math.floor(seconds / 30),
        });
    }
    achievements
      .slice(0, Math.min(achievements.length, 1 + (index % 3)))
      .forEach((achievement) =>
        userAchievements.push({
          id: `seed-user-achievement-${suffix}-${achievement.id}`,
          userId,
          achievementId: achievement.id,
          unlockedAt: dateAgo(index % 20),
        }),
      );
    notifications.push(
      {
        id: `seed-notification-${suffix}-1`,
        userId,
        title: "Mục tiêu hôm nay",
        body: "Bạn còn một bài học ngắn để hoàn thành mục tiêu.",
        type: "reminder",
        href: "/dashboard",
        read: index % 2 === 0,
        createdAt: dateAgo(0),
      ...
      {
        id: `seed-notification-${suffix}-2`,
        userId,
        title: "Chuỗi học tập",
        body: `Bạn đang có chuỗi ${index % 15} ngày liên tiếp.`,
        type: "achievement",
        href: "/analytics",
        read: index % 3 === 0,
        createdAt: dateAgo(2),
      ...
      {
        id: `seed-notification-${suffix}-3`,
        userId,
        title: "Từ vựng cần ôn",
        body: "Một số từ đã đến lịch ôn tập.",
        type: "review",
        href: "/vocabulary",
        read: false,
        createdAt: dateAgo(1),
    ...
    favorites.push({
      id: `seed-favorite-${suffix}-word`,
      userId,
      type: "word",
      resourceId: favoriteWord.id,
      title: favoriteWord.word,
      href: `/dictionary?q=${encodeURIComponent(favoriteWord.word)}`,
      createdAt: dateAgo(index % 10),
    });
    notes.push(
      {
        id: `seed-note-${suffix}-1`,
        userId,
        title: "Cụm từ cần nhớ",
        content: "Ghi chú thử nghiệm về các cụm từ giao tiếp thường ngày.",
        tags: ["giao tiếp", "ôn tập"],
        resourceId: lessons[0].id,
        createdAt: dateAgo(5),
        updatedAt: dateAgo(1),
      },
      {
        id: `seed-note-${suffix}-2`,
        userId,
        title: "Kế hoạch tuần",
        content: "Hoàn thành hai bài học và ôn flashcard mỗi ngày.",
        tags: ["kế hoạch"],
        createdAt: dateAgo(3),
        updatedAt: dateAgo(0),
      },
    ...
    if (premium)
      payments.push({
        ...
        userId,
        providerId: `seed-payment-provider-${suffix}`,
        amount: 149000,
        currency: "VND",
        status: "paid",
        createdAt: dateAgo(index % 25),
      });
    ...
    conversations.push({
      ...
      difficulty: levels[(index - 1) % levels.length],
      correction: "balanced",
      createdAt: dateAgo(index % 7),
      updatedAt: dateAgo(index % 3),
    });
    messages.push(
      {
        id: `seed-message-${suffix}-1`,
        conversationId,
        role: "user",
        content: "Could I have a cup of coffee, please?",
        createdAt: dateAgo(index % 7),
      },
      {
        id: `seed-message-${suffix}-2`,
        conversationId,
        role: "assistant",
        content: "Of course. What kind of coffee would you like?",
        metadata: {
          corrected: false,
    ...
    if (writingExercises.length) {
      ...
      writingSubmissions.push({
        id: `seed-writing-${suffix}`,
        userId,
        exerciseId: exercise.id,
        text: "I am learning English every day because I want to communicate confidently at work and while travelling.",
        analysis: {
          score: 72 + (index % 20),
          strengths: ["clear meaning", "good vocabulary"],
          improvements: ["add more detail"],
        },
        createdAt: dateAgo(index % 14),
      });
    }
  }
  ...
  const batches: Array<[string, Promise<{ count: number }>]> = [
    [
      "enrollments",
      db.courseEnrollment.createMany({
        data: enrollments,
        skipDuplicates: true,
      }),
    ],
    ...
    [
      "vocabulary reviews",
      db.vocabularyReview.createMany({
        data: vocabularyReviews,
        skipDuplicates: true,
      }),
    ],
    [
      "quiz attempts",
      db.quizAttempt.createMany({ data: attempts, skipDuplicates: true }),
    ],
    [
      "achievements",
    ...
    [
      "study sessions",
      db.studySession.createMany({ data: sessions, skipDuplicates: true }),
    ],
    [
      "daily goals",
      db.dailyGoal.createMany({ data: dailyGoals, skipDuplicates: true }),
    ],
    [
      "notifications",
      db.notification.createMany({ data: notifications, skipDuplicates: true }),
    ],
    [
      "favorites",
      db.favorite.createMany({ data: favorites, skipDuplicates: true }),
    ],
    ["notes", db.note.createMany({ data: notes, skipDuplicates: true })],
    [
      "payments",
      db.payment.createMany({ data: payments, skipDuplicates: true }),
    ],
    [
      "AI conversations",
      db.aIConversation.createMany({
        data: conversations,
        skipDuplicates: true,
      }),
    ],
    [
      "writing submissions",
      db.writingSubmission.createMany({
  ...
  const results = await Promise.all(
    batches.map(
      async ([name, operation]) => [name, (await operation).count] as const,
    ),
  );
  const attemptIds = new Set(
    (
      await db.quizAttempt.findMany({
        where: { id: { startsWith: "seed-attempt-" } },
        select: { id: true },
      })
    ).map((item) => item.id),
  );
  ...
  const conversationIds = new Set(
    (
      await db.aIConversation.findMany({
        where: { id: { startsWith: "seed-conversation-" } },
        select: { id: true },
      })
    ).map((item) => item.id),
  );
  ...
  results.push([
    "quiz answers",
    (await db.answer.createMany({ data: validAnswers, skipDuplicates: true }))
      .count,
  ]);
  results.push([
    "AI messages",
  ...
  const testUsers = await db.user.count({
    where: { email: { startsWith: "seed.student." } },
  });
  const totalRows = results.reduce((sum, [, count]) => sum + count, 0);
  console.log(
    `Cloud test seed complete on ${databaseHost}: ${testUsers} test users, ${totalRows} new related rows.`,
  );
  for (const [name, count] of results) console.log(`- ${name}: ${count}`);
}
