...
async function main() {
  ...
  try {
    ...
    assert.equal(
      (
        await anon.request("/api/auth/register", "POST", {
          name: "Bad",
    ...
    const placementAnswers = await db.question.findMany({
      where: {
        id: { in: placement.questions.map((q: { id: string }) => q.id) },
      },
    });
    ...
    assert.equal(
      (await learner.request("/api/lessons/a1-1/progress", "POST", { step: 5 }))
        .status,
      400,
    );
    ...
    const actual = await db.question.findMany({
      where: {
        id: { in: lesson.quiz.questions.map((q: { id: string }) => q.id) },
      },
    });
    const submission = {
    ...
    assert.equal(
      (
        await learner.request("/api/vocabulary/review", "POST", {
          vocabularyId: "v01",
          rating: "good",
        })
      ).status,
    ...
    assert.equal(
      await db.vocabularyReview.count({
    ...
    assert.equal(
      (
        await learner.request(`/api/notes/${foreign.notes[0].id}`, "PATCH", {
          title: "Hijack",
          content: "No",
          tags: [],
        })
    ...
    await learner.json("/api/study/finish", "POST", { sessionId: start.id });
    assert.equal(
      (await learner.json("/api/dashboard")).user.progress.studySeconds,
      savedSeconds,
    );
    ...
    if (!config.ai) {
      assert.equal(
        (
          await fresh.request("/api/ai/chat", "POST", {
            message: "Hello",
      ...
      assert.equal(
        (await fresh.json("/api/ai/status")).usage.chat.used,
        aiStatus.usage.chat.used,
      );
    }
    ...
    const foreignConversation = await db.aIConversation.create({
      data: {
        userId: foreignId,
        title: "Private foreign chat",
        scenario: "Airport",
        difficulty: "Beginner",
        correction: "none",
      },
    ...
    assert(
      (await fresh.json("/api/ai")).some(
        (item: { id: string }) => item.id === conversation.id,
      ),
    );
    assert.equal(
      (await fresh.json(`/api/ai/${conversation.id}`)).messages[0].metadata
        .followUp,
      "How are you?",
    );
    assert.equal(
      (await learner.request(`/api/ai/${foreignConversation.id}`, "DELETE"))
        .status,
      404,
    );
    assert.equal(
      (await learner.request(`/api/ai/${conversation.id}`, "DELETE")).status,
      200,
    );
    assert.equal(
      await db.aIConversation.findUnique({ where: { id: conversation.id } }),
      null,
    );
    if (process.env.DEMO_PASSWORD) {
      ...
      assert.equal(
        (await anon.request(`/api/courses/${course.id}`)).status,
        404,
      );
    ...
    console.log(
      "PASS: registration, auth, RBAC, CSRF, placement, enrollment, lesson progression, concurrent XP, SRS, notes ownership, favorites, study-time idempotency, re-login persistence, AI status/history ownership, admin CRUD, session revocation.",
    );
  } finally {
    for (const id of createdContent)
      await db.course.deleteMany({ where: { id } });
    if (userId) await db.user.deleteMany({ where: { id: userId } });
    if (foreignId) await db.user.deleteMany({ where: { id: foreignId } });
